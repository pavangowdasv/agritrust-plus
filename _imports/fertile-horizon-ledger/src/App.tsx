...
const StatCard = ({ icon: Icon, label, value, colorClass, trend }: { icon: any, label: string, value: string | number, colorClass: string, trend?: string }) => (
  <div className="bg-surface-container-lowest p-5 rounded-xl flex justify-between items-center shadow-sm border border-outline-variant/10">
  ...
  <div className="group">
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div className="mt-2 bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/10">
            {children}
          </div>
        </motion.div>
      )}
...
const QuickActionButton = ({ icon: Icon, label }: { icon: any, label: string }) => (
  <motion.button 
    whileTap={{ scale: 0.95 }}
    className="flex flex-col items-center justify-center gap-3 bg-primary text-white p-6 rounded-xl shadow-lg shadow-primary/10 hover:bg-primary-container transition-colors"
  >
    <Icon size={32} />
    ...
    <div className="min-h-screen bg-background text-on-surface pb-32">
      ...
      <header className="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-md flex items-center justify-between px-6 py-4 border-b border-outline-variant/10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-primary font-extrabold tracking-tight text-2xl leading-tight">Hi, Bank Officer</h1>
            <p className="text-[10px] text-on-surface-variant font-medium">Manage farmer verification, payments, and financial records securely.</p>
          </div>
        </div>
        <div className="flex gap-2 items-center">
          ...
          <button className="md:hidden p-2 rounded-full hover:bg-surface-container-high transition-colors">
      ...
      <main className="max-w-2xl mx-auto pt-24 px-4 space-y-12">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="space-y-3">
              ...
              />
              <div className="bg-error-container/30 p-5 rounded-xl flex justify-between items-center border border-error/10 shadow-sm">
                <div className="flex items-center gap-4">
        ...
        <section className="space-y-4">
          <h2 className="text-2xl font-bold tracking-tight text-on-surface px-1">Officer Management</h2>
          
          <AccordionItem 
            icon={ShieldCheck} 
            title="Verify Farmers" 
            isOpen={openAccordion === 'verify'}
            onClick={() => toggleAccordion('verify')}
          >
            <h3 className="text-xs font-bold text-primary mb-6 uppercase tracking-widest">Farmer Verification</h3>
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              <div>
                <label className="block text-[10px] font-bold text-on-surface-variant mb-1 ml-1 uppercase tracking-wider">Farmer Name</label>
              ...
              <button className="w-full bg-primary text-white font-bold py-4 rounded-xl hover:bg-primary-container transition-colors mt-2 shadow-lg shadow-primary/10">Verify Farmer</button>
            </form>
          </AccordionItem>
          ...
          >
            <h3 className="text-xs font-bold text-primary mb-6 uppercase tracking-widest">Confirm Farmer Payment</h3>
            <div className="space-y-4">
              <input className="w-full bg-surface-container-low border-none rounded-lg p-3 text-sm" placeholder="Farmer Name" type="text"/>
              <input className="w-full bg-surface-container-low border-none rounded-lg p-3 text-sm" placeholder="Transaction ID" type="text"/>
              <div className="grid grid-cols-2 gap-4">
                <input className="w-full bg-surface-container-low border-none rounded-lg p-3 text-sm" placeholder="Amount (ETH/USD)" type="text"/>
          ...
          <AccordionItem 
            ...
            isOpen={openAccordion === 'loans'}
            onClick={() => toggleAccordion('loans')}
          >
            <h3 className="text-xs font-bold text-primary mb-6 uppercase tracking-widest">Loan Processing</h3>
            <div className="space-y-4">
              <input className="w-full bg-surface-container-low border-none rounded-lg p-3 text-sm" placeholder="Farmer Name" type="text"/>
          ...
          <AccordionItem 
            icon={ShieldCheck} 
            title="Insurance and Credit Records" 
          ...
          <AccordionItem 
            ...
            isOpen={openAccordion === 'linkage'}
            onClick={() => toggleAccordion('linkage')}
          >
            <h3 className="text-xs font-bold text-primary mb-6 uppercase tracking-widest">Wallet & Ledger Linkage</h3>
            <div className="space-y-4">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <QuickActionButton icon={UserPlus} label="Verify Farmer" />
            <QuickActionButton icon={CheckSquare} label="Confirm Payment" />
            <QuickActionButton icon={Wallet} label="Process Loan" />
            <QuickActionButton icon={Receipt} label="View Transactions" />
          </div>
        </section>
          ...
          <div className="relative h-64 rounded-2xl overflow-hidden group shadow-2xl">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-primary/90 via-primary/40 to-transparent flex items-end p-8">
              <div className="text-white">
                ...
                <p className="text-sm opacity-90 max-w-xs font-medium">Blockchain-powered transparency for modern agricultural banking.</p>
              </div>
            </div>
          </div>
        </section>
      ...
      <footer className="bg-surface-container-low w-full py-16 mb-20 flex flex-col items-center space-y-8 px-6">
        <div className="flex flex-wrap justify-center gap-x-10 gap-y-4">
          {['About', 'Contact', 'Privacy Policy', 'Terms', 'Support'].map((link) => (
            <a key={link} className="text-on-surface-variant hover:text-primary transition-colors text-[10px] font-bold uppercase tracking-widest" href="#">{link}</a>
          ))}
        </div>
        <div className="text-center space-y-3">
          <p className="text-primary font-extrabold text-sm tracking-tight">Fertile Horizon Blockchain</p>
          <p className="text-on-surface-variant text-[9px] font-bold uppercase tracking-[0.4em] opacity-60">© 2024 Secure Agricultural Ledger.</p>
        </div>
      </footer>
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-8 pt-4 bg-surface/90 backdrop-blur-xl border-t border-outline-variant/10">
        ...
        <button className="flex flex-col items-center justify-center text-on-surface-variant py-2 hover:text-primary transition-colors active:scale-90 duration-200">
          <Tractor size={20} />
          <span className="text-[10px] font-bold mt-1">Farmers</span>
        </button>
        <button className="flex flex-col items-center justify-center text-on-surface-variant py-2 hover:text-primary transition-colors active:scale-90 duration-200">
          <CreditCard size={20} />
